...
export function EditTransactionSheet({ transaction, onClose }: Props) {
  ...
  const toggleBg = isDark ? '#262730' : '#F5F4F2';
  const activeTabBg = isDark ? '#3A3B44' : '#FFFFFF';
  const inactiveColor = isDark ? '#5C5F69' : '#ABABAB';
  const deleteBg = isDark ? '#3D1517' : '#FFEBEE';
        ...
        >
          ...
          >
            ...
            <View style={styles.fields}>
              {/* Type Toggle */}
              <View style={[styles.typeToggle, { backgroundColor: toggleBg }]}>
                <TouchableOpacity
                  style={[
                    styles.typeTab,
                    type === 'expense' && [styles.typeTabActive, { backgroundColor: activeTabBg }],
                  ]}
                ...
                >
                  <Ionicons
                    name="arrow-up"
                    size={16}
                    color={type === 'expense' ? '#E53935' : inactiveColor}
                  />
                  <Text style={[styles.typeTabText, { color: type === 'expense' ? colors.text : inactiveColor }]}>
                    Expense
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.typeTab,
                    type === 'income' && [styles.typeTabActive, { backgroundColor: activeTabBg }],
                  ]}
                ...
                >
                  <Ionicons
                    name="arrow-down"
                    size={16}
                    color={type === 'income' ? '#2E7D32' : inactiveColor}
                  />
                  <Text style={[styles.typeTabText, { color: type === 'income' ? colors.text : inactiveColor }]}>
                    Income
                  </Text>
                </TouchableOpacity>
              </View>
                ...
                {showCategoryPicker && (
                  <View style={[styles.categoryList, { backgroundColor: sheetBg, borderColor: inputBorder }]}>
                    {categoriesLoading ? (
                      ...
                      <Text style={[styles.categoryItemText, { color: subtitleColor, padding: 16 }]}>No categories</Text>
                    ) : (
                      categories.map((cat: CategoryResponse) => (
                        <TouchableOpacity
                          ...
                          style={[
                            ...
                            String(cat.id) === categoryId && { backgroundColor: '#2E7D3212' },
                          ]}
                          onPress={() => {
                            setCategoryId(String(cat.id) === categoryId ? '' : String(cat.id));
                            setShowCategoryPicker(false);
                          }}
                          activeOpacity={0.7}
                        >
            ...
            <View style={styles.buttonArea}>
              <TouchableOpacity
                style={[styles.saveBtn, (isUpdating || isDeleting) && { opacity: 0.6 }]}
                onPress={handleSave}
              ...
              <TouchableOpacity
                style={[styles.deleteBtn, { backgroundColor: deleteBg }, isDeleting && { opacity: 0.6 }]}
...
const styles = StyleSheet.create({
  ...
  textInput: { height: 52, borderRadius: 14, borderWidth: 1, paddingHorizontal: 18, justifyContent: 'center' },
  textInputField: { fontSize: 16, fontWeight: '500', fontFamily: 'Outfit-Medium', flex: 1 },

  categoryList: { borderRadius: 14, borderWidth: 1, overflow: 'hidden', maxHeight: 200 },
  categoryItem: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  buttonArea: { paddingHorizontal: 28, paddingTop: 24, gap: 12 },
  saveBtn: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  deleteBtnText: { color: '#E53935', fontSize: 14, fontWeight: '500', fontFamily: 'Outfit-Medium' },
});
